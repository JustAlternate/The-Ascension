...
# Simule les inputs
var input_move_right = false
var input_move_left = false
var input_move_up = false
...
func get_input():
	if alive:
		...
		if sur_echelle and input_move_up:
			velocity.y = -vitesse_dechelage

		if is_on_floor() and input_move_up:
...
func move_box():
	if input_interact and pushing != null:
